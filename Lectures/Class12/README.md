...
#### ViewData & ViewBag
	...
	2. `ViewBag` - Provides dynamic access to the to the objects stored in `ViewData`. 
...
Note that the data inside IS THE SAME <br />
`ViewData["Title"] = "MyTitle";` <br />
`ViewBag.Title; // returns "MyTitle";` <br />
1. When to use one or another?
	- ViewBag doesn't support not valid C# names. you can't access ViewData["Key With Space"] with ViewBag
	- ViewBag.Something is dynamic and you may have problems when calling methods (like extension methods) 
	that needs to know the exact parameter at compile time.
...
2. Knowing the subtle differences, using one or another is much more a taste preference.
3. Normally you can think of ViewBag.AnyKey to an alias of ViewData["AnyKey"]
